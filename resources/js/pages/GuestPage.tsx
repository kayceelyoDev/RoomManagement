...
import {
    Search, Calendar, Users, ArrowRight, LogOut, Menu, X,
    BedDouble, CheckCircle, ClipboardList, Star
} from 'lucide-react';
...
interface Room {
    ...
    // Added reservations here so the Modal works correctly
...
export default function GuestPage({ rooms = [], services = [] }: GuestPageProps) {
    ...
    const handleProceedToReservation = () => {
        // This closes the "Details" popup and opens the "Booking" form
        // Since the AddReservation component is rendered conditionally below,
        // we just need to flip the switches.
        setIsReservationModalOpen(true);
    };

    const handleCloseReservation = () => {
        setIsReservationModalOpen(false);
        // Optional: clear selected room if you want them to start over
            ...
            <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100 transition-all">
                <div className="px-6 py-3 flex items-center justify-between max-w-7xl mx-auto">
                    <Link href='/' className="flex items-center gap-2 hover:opacity-80 transition">
                        <img src="/img/logo.jpg" alt="Estaca Bay Logo" className="h-10 w-10 object-contain rounded-full border border-gray-200" />
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3 pl-4 border-l border-gray-200">
                            ...
                            <Link href={logout()} method="post" as="button" className="p-2 text-gray-500 hover:text-red-600 transition rounded-full hover:bg-red-50" title="Log Out">
                                <LogOut size={20} />
                            </Link>
                        </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
                        ...
                        <div className="flex w-full lg:w-auto gap-2 overflow-x-auto pb-2 lg:pb-0">
                            <div className="flex items-center gap-2 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-600 cursor-pointer hover:border-[#628141] transition min-w-[140px]">
                                <Calendar size={16} className="text-[#628141]" /> <span>Check-in</span>
                            </div>
                            <div className="flex items-center gap-2 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-600 cursor-pointer hover:border-[#628141] transition min-w-[140px]">
                                <Calendar size={16} className="text-[#628141]" /> <span>Check-out</span>
                            </div>
                        </div>
                            ...
                            {categories.map(cat => (
                                <button
                                    ...
                                    className={`whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wide transition-all ${
                                        selectedCategory === cat ? 'bg-[#2C3930] text-[#FFFDE1] shadow-md transform scale-105' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                                    }`}
                                >
                                    {cat}
                                </button>
                    ...
                    {filteredRooms.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredRooms.map((room) => (
                                <div key={room.id} className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl hover:border-[#D8E983]/50 transition-all duration-300 flex flex-col h-full">
                                    <div className="relative h-56 overflow-hidden">
                                        <div className="absolute inset-0 bg-black/10 group-hover:bg-transparent transition-colors z-10" />
                                        <img src={getRoomImage(room.img_url)} alt={room.room_name} className="w-full h-full object-cover transform group-hover:scale-110 transition duration-700" loading="lazy" />
                                    ...
                                    <div className="p-5 flex flex-col flex-1">
                                        ...
                                        <div className="flex items-center gap-4 text-xs text-gray-500 mb-4 bg-gray-50 p-3 rounded-lg">
                                            <div className="flex items-center gap-1.5"><Users size={14} className="text-[#628141]" /> <span>{room.room_category?.room_capacity} Guests</span></div>
                                            <div className="w-px h-3 bg-gray-300" />
                                            <div className="flex items-center gap-1.5"><BedDouble size={14} className="text-[#628141]" /> <span className="truncate max-w-[80px]">{room.type_of_bed}</span></div>
                                        </div>

                                        <div className="mt-auto flex items-center justify-between pt-4 border-t border-gray-100">
                                            <div>
                                                <span className="text-xs text-gray-400 block">Price per night</span>
                                                <span className="text-lg font-bold text-[#628141]">{room.room_category && formatCurrency(room.room_category.price)}</span>
                                            </div>
                                            <button onClick={() => setSelectedRoom(room)} className="px-6 py-2.5 bg-[#2C3930] text-white text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-[#628141] transition shadow-md hover:shadow-lg active:scale-95">
                                                Book Now
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200">
                            ...
                            <button onClick={() => { setSearchTerm(''); setSelectedCategory('All'); }} className="mt-4 text-[#628141] font-bold text-sm hover:underline">Clear all filters</button>
                        </div>
                    )}
                </div>
            </main>
            ...
            {selectedRoom && !isReservationModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#2C3930]/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl relative flex flex-col md:flex-row animate-in slide-in-from-bottom-4 duration-300">
                        <button onClick={() => setSelectedRoom(null)} className="absolute top-4 right-4 z-20 p-2 bg-white/80 hover:bg-white rounded-full text-gray-800 transition shadow-md"><X size={20} /></button>
                        ...
                        <div className="w-full md:w-5/12 h-64 md:h-auto bg-gray-100 relative">
                            ...
                            <div className="absolute bottom-6 left-6 text-white">
                                <span className="px-2 py-1 bg-[#D8E983] text-[#2C3930] text-[10px] font-bold uppercase rounded-md mb-2 inline-block">{selectedRoom.room_category?.room_category}</span>
                        ...
                        <div className="w-full md:w-7/12 p-8 overflow-y-auto bg-white flex flex-col">
                            <div className="mb-6">
                                <h4 className="text-sm font-bold text-[#2C3930] uppercase tracking-wider mb-2 flex items-center gap-2"><CheckCircle size={16} className="text-[#628141]" /> Room Details</h4>
                                <p className="text-sm text-gray-600 leading-relaxed">{selectedRoom.room_description}</p>
                            </div>

                            <div className="grid grid-cols-2 gap-4 mb-6">
                                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                                    <span className="text-[10px] text-gray-400 uppercase font-bold">Capacity</span>
                                    <div className="flex items-center gap-2 text-sm font-bold text-[#2C3930] mt-1"><Users size={16} /> {selectedRoom.room_category?.room_capacity} Persons</div>
                                </div>
                                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                                    <span className="text-[10px] text-gray-400 uppercase font-bold">Bed Configuration</span>
                                    <div className="flex items-center gap-2 text-sm font-bold text-[#2C3930] mt-1"><BedDouble size={16} /> {selectedRoom.type_of_bed}</div>
                                </div>
                            </div>
                                ...
                                <div className="mb-8">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {selectedRoom.room_amenities.split(',').map((amenity, idx) => (
                                            <span key={idx} className="text-xs px-3 py-1.5 bg-[#FFFDE1] text-[#2C3930] rounded-lg border border-[#D8E983] font-medium">{amenity.trim()}</span>
                                        ))}
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="mt-auto pt-6 border-t border-gray-100">
                                <div className="flex items-center justify-between mb-4">
                                    <div>
                                        ...
                                        <div className="text-3xl font-bold text-[#628141]">{selectedRoom.room_category && formatCurrency(selectedRoom.room_category.price)}</div>
                                    </div>
                                ...
                                <button className="w-full py-4 bg-[#2C3930] text-[#FFFDE1] rounded-xl font-bold uppercase text-sm tracking-widest hover:bg-[#3E4F42] transition shadow-xl flex items-center justify-center gap-2" onClick={handleProceedToReservation}>
